 ...
 * Enhanced version with comprehensive CRUD (Create, Read, Update, Delete) functionalities
 * and CSV-based data persistence for employee records.
 * Features: Employee Management, Department Management, Attendance Tracking,
 * Payslip Generation, Leave Request Management.
 */
public class Project extends JFrame {
    ...
    private JMenuBar createMenuBar() {
        ...
        JMenuItem saveDataItem = new JMenuItem("Save Employee Data");
        JMenuItem exitItem = new JMenuItem("Exit");

        saveDataItem.addActionListener(e -> dataManager.saveEmployeesToCSV());
    ...
    private void showAboutDialog() {
        JOptionPane.showMessageDialog(this,
            "MotorPH Employee Management System\n" +
            "Version 2.1 - With CSV Persistence\n" +
    ...
    class EmployeeDataManager {
        ...
        public EmployeeDataManager() {
            ...
            loadSampleData(); // For other modules
        }

        private void loadEmployeesFromCSV() {
            File file = new File(EMPLOYEES_CSV_FILE);
            // Create file with headers if it doesn't exist
            if (!file.exists()) {
                try (PrintWriter writer = new PrintWriter(new FileWriter(EMPLOYEES_CSV_FILE))) {
                    writer.println("ID,FirstName,LastName,BirthDate,Position,Department,Salary,ContactNumber,Address");
                    // Add some default data for a fresh start
                    writer.println("1001,Alice,Williams,1990-05-15,Software Developer,IT,55000.0,09171234567,123 Maple St");
                    writer.println("1002,Bob,Davis,1988-08-22,HR Manager,HR,65000.0,09187654321,456 Oak Ave");
                } catch (IOException e) {
                    e.printStackTrace();
                    JOptionPane.showMessageDialog(null, "Could not create employee data file.", "Error", JOptionPane.ERROR_MESSAGE);
                }
            }
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(file))) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if (values.length >= 9) {
                        try {
                            ...
                            emp.setSalary(Double.parseDouble(values[6].trim()));
                            emp.setContactNumber("N/A".equals(values[7].trim()) ? "" : values[7].trim());
                            emp.setAddress("N/A".equals(values[8].trim()) ? "" : values[8].trim());
                            employees.add(emp);
                ...
                e.printStackTrace();
                JOptionPane.showMessageDialog(null, "Error loading employee data from CSV: " + e.getMessage(), "Load Error", JOptionPane.ERROR_MESSAGE);
            }
        }
        ...
        public void saveEmployeesToCSV() {
            try (PrintWriter writer = new PrintWriter(new FileWriter(EMPLOYEES_CSV_FILE))) {
                ...
                for (Employee emp : employees) {
                    writer.println(emp.toCSVString());
                }
                JOptionPane.showMessageDialog(null, "Employee data saved successfully to " + EMPLOYEES_CSV_FILE, "Success", JOptionPane.INFORMATION_MESSAGE);
            } catch (IOException e) {
                e.printStackTrace();
                JOptionPane.showMessageDialog(null, "Error saving employee data to CSV: " + e.getMessage(), "Save Error", JOptionPane.ERROR_MESSAGE);
            }
        }
        ...
        private void persistEmployees() {
            try (PrintWriter writer = new PrintWriter(new FileWriter(EMPLOYEES_CSV_FILE))) {
                ...
                e.printStackTrace();
                JOptionPane.showMessageDialog(null, "Automatic save failed: " + e.getMessage(), "Save Error", JOptionPane.ERROR_MESSAGE);
            }
        }

        private void loadSampleData() {
            // Sample data for non-persistent modules
    ...
    public static class Employee {
        ...
        public Object[] toTableRow() {
            return new Object[]{employeeID, getFullName(), position, department,
                String.format("₱%.2f", salary), contactNumber != null && !contactNumber.isEmpty() ? contactNumber : "N/A"};
        }
        
        public String toCSVString() {
            String csvContact = (contactNumber == null || contactNumber.trim().isEmpty()) ? "N/A" : contactNumber;
            String csvAddress = (address == null || address.trim().isEmpty()) ? "N/A" : address;
            return String.join(",",
    ...
    // Other domain classes (Department, AttendanceRecord, etc.) remain unchanged
    ...
    class EmployeePanel extends JPanel {
        ...
        public EmployeePanel() {
            ...
            JPanel rightPanel = createRightPanel(); // Contains table and status
        ...
        private JPanel createEmployeeForm() {
            ...
            gbc.gridx = 0; gbc.gridy = 6; formPanel.add(createStyledLabel("Contact #:"), gbc);
            gbc.gridx = 1; formPanel.add(contactField, gbc);

            gbc.gridx = 0; gbc.gridy = 7; formPanel.add(createStyledLabel("Address:"), gbc);
            gbc.gridx = 1; formPanel.add(addressField, gbc);

            gbc.gridx = 0; gbc.gridy = 8; formPanel.add(createStyledLabel("Salary (₱):"), gbc);
            ...
            saveButton.addActionListener(this::saveOrUpdateEmployee);
        ...
        private JPanel createEmployeeTable() {
            ...
            headerPanel.setBackground(new Color(51, 98, 140));
            headerPanel.setPreferredSize(new Dimension(0, 50));
            JLabel headerLabel = new JLabel("  Employee Records");
            headerLabel.setFont(new Font("Segoe UI", Font.BOLD, 16));
            headerLabel.setForeground(Color.WHITE);
        ...
        private JPanel createStatusPanel() {
            ...
            statusMainPanel.setBorder(BorderFactory.createEmptyBorder(10, 0, 0, 0));

            JPanel statusHeaderPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
            statusHeaderPanel.setBackground(new Color(108, 117, 125)); // A neutral gray
            ...
            resultArea.setText("Ready to manage employee records. Select an employee to edit or clear the form to add a new one.");

            JScrollPane resultScroll = new JScrollPane(resultArea);
            resultScroll.setBorder(BorderFactory.createLineBorder(new Color(222, 226, 230), 1));
        ...
        private void saveOrUpdateEmployee(ActionEvent e) {
            try {
                ...
                if (firstName.isEmpty() || lastName.isEmpty() || position.isEmpty() || department.isEmpty()) {
                    throw new IllegalArgumentException("First Name, Last Name, Position, and Department are required.");
                }
                ...
                // If ID field is not editable, we are in update mode.
                if (!idField.isEditable()) {
                    dataManager.updateEmployee(emp);
                    resultArea.setText("SUCCESS: Employee " + emp.getFullName() + " has been updated.");
                } else { // Otherwise, we are in create mode.
                    if (dataManager.findEmployeeById(id) != null) {
                        throw new IllegalArgumentException("Employee ID " + id + " already exists. Please use a different ID.");
                    }
                    dataManager.addEmployee(emp);
                    resultArea.setText("SUCCESS: Employee " + emp.getFullName() + " has been added with ID " + id + ".");
                }
                ...
                resultArea.setText("ERROR: Please enter a valid number for Employee ID and Salary.");
            } catch (Exception ex) {
        ...
        private void deleteEmployee(ActionEvent e) {
            ...
            if (viewRow >= 0) {
                ...
                int confirm = JOptionPane.showConfirmDialog(this,
                    "Are you sure you want to delete '" + employeeName + "' (ID: " + employeeId + ")?",
                    "Confirm Delete",
                ...
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    refreshEmployeeTable();
                    clearForm();
                    resultArea.setText("✅ Employee " + employeeName + " deleted successfully.");
                }
            } else {
                resultArea.setText("❌ Please select an employee from the table to delete.");
            }
        }

        private void clearForm() {
            idField.setText("");
            idField.setEditable(true); // Allow editing for new employee
            ...
            resultArea.setText("Form cleared. Ready to add a new employee.");
        }
        ...
        private void loadSelectedEmployee() {
            ...
            if (viewRow >= 0) {
                ...
                Employee emp = dataManager.findEmployeeById(employeeId);
                if (emp != null) {
                    idField.setText(String.valueOf(emp.getEmployeeID()));
                    idField.setEditable(false); // Prevent editing ID of an existing employee
                    ...
                    salaryField.setText(String.format("%.2f", emp.getSalary()));
                    contactField.setText(emp.getContactNumber() != null ? emp.getContactNumber() : "");
                    addressField.setText(emp.getAddress() != null ? emp.getAddress() : "");
                    resultArea.setText("Loaded details for " + emp.getFullName() + ". You can now update the record.");
                }
            }
        }
    ...
    // Other panel classes (DepartmentPanel, etc.) remain unchanged for now
